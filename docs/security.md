...
| Prompt injection widening permissions | Runtime executables, verification profiles, hooks, project roots, and completion gates are configuration/supervisor-owned; agents cannot select profile, command, executable, argv, working directory, environment, or timeout |
...
| Forged or node-invoked administrator recovery | Recovery HTTP is cookie admin + antiforgery (`/api`) or bearer (`/api/v1`). Actor is `ClaimTypes.NameIdentifier` or Identity name from the authenticated principal, never a body field. Node credentials cannot call diagnosis, start, recheck, confirm-manual, or resume. |
| Claim-token leak in recovery UI/API | Diagnosis and operation DTOs omit claim tokens. Manual evidence and audit JSON never echo tokens, credentials, environment dumps, file contents, or unbounded logs. Hub recovery messages carry the token only as an assignment fence between node and control plane. |
| `force=true` unlock | There is no force flag. `confirm-manual` requires exact project name, version/attempt fences, three true acknowledgements, process-stop evidence, reservation/event gap accounting, and a ≤15-minute owning-workspace repository snapshot. Unresolved writer access and unavailable repository status cannot be waived. |
| Stale or uncertain recovery proof | Proof must match current recovery id/attempt/assignment/binding and a still-valid claim token. Known-zero inventories only; unknown is not empty. Stale observation is `recovery_evidence_stale`. Operator attestation provenance is `operator-attestation`, never node proof. |
| Non-Linux or missing `setsid` | Stop is `process_stop_unproven`. Tree kill, silence, disconnect, lease expiry, and node-service restart are not proof. |
| Deleting state to unstick recovery | Never delete workspaces, Control Plane SQLite, node journals, `Node:EventSpoolPath`, or recovery hold/operation/idempotency/audit rows. Never wipe the spool to fake a flushed proof. |
...
- Project recovery HTTP derives the administrator actor from the authenticated principal. CSRF applies to cookie `/api` mutations. Start body is `InventoryRevision` and `IdempotencyKey` only; the audit description is server-authored. Recheck does not resume the queue. Resume clears only a `Recovered` hold after captured assignments are terminal and captured reservations are `Released`.
- Manual `confirm-manual` is operator attestation, not node-observed quiescence. It retains exact project name, attempt/version fences, acknowledgements, process-stop evidence, reservation/event gap accounting, and a fresh owning-workspace snapshot; it does not collect an operator-authored reason. Fencing-token rotation cannot stop an open process or file descriptor. Reservation force-release is not a substitute for recovery.
- Linux recovery proof enumerates `/proc` by journaled session/process group (PID + start ticks). Non-Linux is conservative (`process_stop_unproven`). Contradictory writer evidence keeps or reinstates the hold.
- Linked retry (`OriginalRequestId`) copies no execution authority. Do not log or return claim tokens on recovery diagnosis.
...
- Verification runs inside Bubblewrap with networking and the host process table isolated, user homes/runtime sockets hidden, host root read-only, and only the canonical repository plus a temporary HOME writable. Baseline checks do not execute repository-provided code. Project checks execute only node-trusted definitions selected by the administrator. A pass is reusable only for the exact verification fingerprint and policy revision, including Git executable mode. Catalog, UI, and `verification.command.started` never expose environment values, credentials, executables, argv, or process output.
...
- Model selectors are `<provider>/<model>`: the reserved prefixes `claude-code`, `antigravity`, and `muse` select their official-harness adapters; every other valid prefix goes **only** to Pi as the runtime adapter (`codex` aliases Pi's `openai-codex`, all others pass through identically). Every runtime selector names an explicit native model id (`codex/gpt-5.6-sol`, `claude-code/fable-5-1`, `antigravity/gemini-3-pro`, `muse/muse-spark-1.3`); `AgentModelSelector.TryParse` rejects a model id that is exactly `default`, and adapters always forward that native id. There is no path from a selector to an arbitrary executable, and an unknown or unauthenticated Pi provider fails closed rather than falling back.
...
Never delete or truncate workspace files, uncommitted changes, branches, Control Plane SQLite, the node assignment journal, `Node:EventSpoolPath`, or recovery rows as a shortcut. Never pass `force=true`, skip inventory fencing, or skip process-stop evidence, exact-name confirmation, or required acknowledgements on manual recovery.
