...
- Project recovery HTTP derives the administrator actor from the authenticated principal. CSRF applies to cookie `/api` mutations. Start sets the hold before `RecoverAssignment` delivery. Recheck does not resume the queue. Resume clears only a `Recovered` hold after captured assignments are terminal and captured reservations are `Released`.
- Manual `confirm-manual` is operator attestation, not node-observed quiescence. Fencing-token rotation cannot stop an open process or file descriptor. Reservation force-release is not a substitute for recovery.
...
Never delete or truncate workspace files, uncommitted changes, branches, Control Plane SQLite, the node assignment journal, `Node:EventSpoolPath`, or recovery rows as a shortcut. Never pass `force=true` or skip confirmations.
