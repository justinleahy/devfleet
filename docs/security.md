...
Typed-options defaults (when unset) are under `~/.config/pi-command-center/`. `scripts/setup-local.sh` instead writes `$PI_CC_DATA` (`~/.local/share/pi-command-center` unless overridden):

| File | Role |
|---|---|
| `$PI_CC_DATA/admin.password.hash` | `Admin:PasswordFile` (Identity hash, `0600`) |
| `$PI_CC_DATA/admin.password` | One-time plaintext for the operator (`0600`); not what the runtime loads |
| `$PI_CC_DATA/node.token` | `NodeAuthentication:CredentialFile` (256-bit hex, `0600`) |
| `$PI_CC_DATA/local.env` | Sourced by `scripts/demo.sh` |
| `$PI_CC_DATA/pi-command-center.env` | systemd `EnvironmentFile=` |
...
- Claude: settings and hook live under `$XDG_DATA_HOME/pi-command-center/claude-runtime/<session>/`; `--setting-sources ""` prevents merged project or user hooks, while the explicit host `--settings` file installs the reservation gate.
- Pi reserved tools must present lease id + fencing token; the node calls `AuthorizeMutation` immediately before the write.
- Antigravity runs inside Bubblewrap with the entire host root read-only. The explicit repository read-only bind remains effective even through workspace symlinks.
