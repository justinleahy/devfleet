...
/// <summary>
/// Focused start-flow contract: AgentStartRequest prompt, mode, parent session id, model, and
/// system prompt must reach the worker's <c>session.start</c>/<c>session.input</c> frames.
/// </summary>
public sealed class PiRuntimeAdapterStartFlowTests
{
    ...
    public async Task Root_start_flows_prompt_and_metadata_then_sends_first_input()
    {
        ...
        Assert.False(result.StartPayload?.TryGetProperty("model", out _));
    ...
    [Fact]
    public async Task Model_and_system_prompt_options_flow_into_session_start()
    {
        var options = DefaultOptions();
        options.Model = "pi-large";
        ...
        Assert.Equal("pi-large", result.StartPayload?.GetProperty("model").GetString());
